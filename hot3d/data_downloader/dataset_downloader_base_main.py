...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=False,
        help="""
        If set, re-download and overwrite sequences which have been downloaded before
        """,
    )
    ...
    parser.add_argument(
        ...
        If not set, and IFF single data type exists, it will will be downloaded automatically",
        """,
    )
...
def main():
    ...
    if len(args.data_types) == 0 and len(all_data_groups_list) > 1:
        ...
        args.data_types = [i for i in range(len(all_data_groups_list))]
    ...
    downloader = DatasetDownloaderBase(
